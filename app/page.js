...
export default function Home() {
  ...
  const handleModelSelect = async (model) => {
    ...
    // درخواست به API و ارسال defaultMessage حذف شد
    await get_history(); // فقط تاریخچه رو آپدیت می‌کنیم
  };

  return (
    <div className="flex bg-[url('https://example.com/grass-background.jpg')] bg-cover bg-center min-h-screen">
      <nav className="hidden md:block fixed top-0 border-b border-gray-700 left-0 right-0 bg-gray-800 md:max-w-[82.5%] sm:max-w-[90%] shadow-md z-50">
      ...
      <div className="flex-1 max-w-[1200px] m-auto text-center min-h-screen flex flex-col justify-between">
        <div>
          ...
          <div className="mt-[100px] md:mt-[70px] px-4">
            ...
            <div className="chat-container overflow-y-auto overflow-x-hidden p-[10px] max-h-[calc(100vh-200px)] bg-[url('https://example.com/grass-background.jpg')] bg-cover bg-center">
              {chats
                .concat(
                ...
                .map((chat, index) => (
                  <div
                    key={index}
                    className={`message m-[10px] w-[70%] p-[10px] rounded-lg flex items-start gap-x-2 ${
                      chat.sender === 'user'
                        ? 'user-message bg-[#007bff] text-white ml-auto text-right'
                        : 'ai-message bg-transparent text-base/8 px-4 text-right text-white'
                    }`}
                    style={{ display: 'block', width: 'fit-content', maxWidth: '100%', overflowX: 'hidden', minHeight: '50px' }}
                  >
                    {chat.sender !== 'user' && (
