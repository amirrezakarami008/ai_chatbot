...
import { Menu, X, ChevronDown, ChevronUp, UsersRound, Headset, LogOut, History, UserRound } from 'lucide-react';
...
export default function Home() {
    ...
    try {
      ...
      if (response) {
        ...
        const validConversations = conversations.filter(
          (chat) => chat && chat.sender && chat.text && chat.text.trim() !== ''
        );
  ...
  const get_messagess = async () => {
    if (selectedHistoryId) {
      ...
      return response; // برای استفاده در useEffect
    }
    return null;
  };
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      if (selectedHistoryId) {
        try {
          ...
          const response = await get_messagess(); // فراخوانی تابع
          console.log('پیام‌ها:', response);
  
          // فرض می‌کنیم response یه آرایه از پیام‌ها رو برگردونده
          if (response && Array.isArray(response)) {
            const formattedMessages = response.map((msg) => ({
              sender: msg.sender === 'me' ? 'user' : 'ai', // تبدیل 'me' به 'user' و بقیه به 'ai'
              text: msg.text || 'پیام خالی',
            }));
            setChats(formattedMessages); // آپدیت chats با پیام‌های جدید
          } else {
            throw new Error('فرمت داده‌های دریافتی نامعتبر است');
          }
        } catch (error) {
    ...
    fetchMessages(); // اجرای تابع
  }, [selectedHistoryId]); // وابستگی به selectedHistoryId
  ...
  useEffect(() => {
    ...
    if (!storedToken) {
      ...
      get_history(); // فقط یک بار موقع لود یا تغییر توکن اجرا می‌شه
    }
  }, [router]); // فقط وابستگی به router

  // برای اسکرول موقع تغییر chats
  useEffect(() => {
    scrollToBottom();
  }, [chats]);

  // برای نمایش نوتیفیکیشن اولیه
  useEffect(() => {
    // addNotification('تاریخچه و هوش مصنوعی‌های دیگر فعلاً در دسترس نیست و طی چند روز آینده همه هوش مصنوعی‌ها و تاریخچه اضافه خواهد شد پس از هوشیار استفاده کن و لذت ببر');
  }, []);
  ...
  {chats
    .concat(isTyping ? [{ sender: 'ai-typing', text: typingText + (isTyping ? '|' : '') }] : [])
    .filter((chat) => chat && chat.text && chat.text.trim() !== '') // فقط چت‌های معتبر رندر می‌شن
    .map((chat, index) => (
      <div
        key={index}
